...
2. Rather than blindly accept that points are spatially autocorrelated, you should run a Mantel Test before you build your model to discover that there is autocorrelation that must be accounted for. To do this, first, create a distance matrix using the `dist()` function in R. The only argument you will need to provide is two joined columns of 'east' and 'north' from the main data set (these represent distances east and north from a corner of the common garden). Then create a second matrix using the observed seed biomass data, 'seeds'. Inspect the matrices using `as.matrix()`. Use the `mantel.rtest()` function in the `ade4` library (you will need to install and load this) to test for spatial autocorreltation in the data.  Remember, all this indicates is that your model will need to account for auto-correlation in the residuals.
...
<div class="hider">
Plots that are less than 10 meters apart covary at least somewhat. Plots that are within ~4 meters of each other covary considerably.
</div>
