...
We have created a data frame called 'dat' with one response, two covariates and a shared spatial position for each observation in utms. Data like this are usually where you will 
start. 
...
Fit an aspatial model in JAGS predicting the response ($y$) as a linear function of the covariates ($X2,X3$). Call your JAGS model `AspatialJAGS.R`. You must use matrix notation for the deterministic model. Review the nitrous oxide problem for the syntax, here is the math you will need:
\begin{align}
