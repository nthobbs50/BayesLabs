...
#### I. Motivation
Many graduate students at CSU perform designed experiments to understand causation.  We often turn to ANOVAs or ANCOVAs to analyze the effects of treatments in these experiments. The Bayesian methods you have learned in this class can be usefully applied to these problems. We often need the Bayesian approach to account for errors in reponses, to deal with group level effects, and, as illustrated here, to deal with spatial autocorrelation. This lab provides illustrates the analysis of a designed experiment with a data model for an unobserved state and continuous spatial depdence in the data. 
...
You will need install the packages `ade4` and `matrixStats`.

#### IV. Calibration model

To inform the calibration model, we counted and weighed seeds from a subset of experimental plots (see *calibration* data set). 

1. We have written the likelihood for the relationship between seed count and seed weight, where seed weight is the response of interest. As seed weight can only be positive, it is lognormally distributed. We assume a linear relationship between seed weight and seed count with an intercept that crosses zero (*i.e.* there is no intercept parameter, just a slope).

$$\text{log}(w_i) \sim \text{normal}(\text{log}(\alpha_c x_{c,i}), \sigma^2_c)$$
...
calib <- read.csv("/Users/Tom/Documents/EcologicalModelingCourse/2017SpringSemester/ESS_575_2017/Labs/SpatialModeling/SeedCt_subset_model.csv")
y_wt <- calib$wt
x_ct <- calib$ct
...
gelman.diag(zc.cal)
```
3. What is $\sigma^2_c$ and what scale is it on? Does this matter for understanding the relationship between seed weight and count? 
<div class="hider">
This is the calibration variance on the log-scale. This doesn't matter for our understanding because we are most interested in the slope, which is not on the log-scale. 
</div>
...
The response, $y_i$ is the observed count in plot $i$ divided by 100, i.e. the units of $y$ are 100's of seeds. You may reasonably treat "counts" as a continuous random variable because the observations in each plot number in the 10,000s. Compose a deterministic model that includes an intercept and the effects of diversity, region, and their interaction.

1. Draw a DAG and write the posterior and joint distribution for the observed and unobserved quantities. Your model will include:
+  A data model relating the observed biomass to unobserved seed "counts".
+  A process model that relates "counts," to the parameters in the deterministic model and the design matrix.  Assume that there is continuous spatial dependence among the plots that must be modeled. You may assume that unstructured variance = 0 because there is no measurement error or fine-scaled heterogeneity at the plot scale.


<div class="hider">
\begin{align}
\Sigma_{x,y} &= \sigma^2{unstr}\mathbf{I} \sigma^2_{str} e^{(\frac{-D_{i,j}}{\phi})}\\
[\phi,\sigma_p^2,\boldsymbol {\beta},\alpha,\sigma^2_c \mid \mathbf y] & \propto \prod_{i=1}^n[y_i\mid \alpha z_i,\sigma^2_c][\mathbf z\mid \mathbf X \boldsymbol \beta, \boldsymbol \Sigma][\boldsymbol \beta,\sigma^2_p,\sigma^2_c,\phi]\\
\text{log}(y_i) & \sim \text{normal}(\text{log}(\alpha_c z_i), \sigma^2_c)\\
\text{log}(\mathbf{z}) & \sim \text{multivariate normal}(\text{log}(\mathbf{X}\boldsymbol \beta), \Sigma)\\
 &\text{with appropriate priors on parameters}
\end{align}
</div>

2. Rather than blindly accept that data points are spatially autocorrelated, you should run a Mantel Test before you build your model to discover that there may be autocorrelation that must be accounted for. To do this, first, create a distance matrix using the `dist()` function in R. The only argument you will need to provide is two joined columns of 'east' and 'north' from the main data set (these represent distances east and north from a corner of the common garden). Then create a second matrix using the observed seed biomass data, 'seeds'. Inspect the matrices using `as.matrix()`. Use the `mantel.rtest()` function in the `ade4` library (you will need to install and load this) to test for spatial autocorreltation in the *data*.  Remember, however, that it is spatial autocorrelation in the *residuals* that we must be concerned about to assure that iid assumptions are met.  It may be that your deterministic model sufficeintly accounts for variation in responses over space over space such that residuals are not spatially structured.

What do these matrices represent? What is this test actually doing? Interpret your results.

```{r, include = key, echo = key, eval = key}
brte_sub <- read.csv("/Users/Tom/Documents/EcologicalModelingCourse/2017SpringSemester/ESS_575_2017/Labs/SpatialModeling/brte_labdata1.csv")
...
```
<div class="hider">
The first matrix looks at the distance between plots in the common garden (differences in values of east and north), while the second matrix looks at the difference in values of seed biomass). The Mantel test determines if there is a correlation between distance between plots and differences in seed biomass measurements between plots. A p-value less than 0.05 rejects the null hypothesis that there is no relationship between the two matrices and the value r reported represents the correlation. Here, we can reject the null and suggest that there is spatial auto-correlation with a correlation of r = `r Mantel$obs`. 

Remember that Bayesians don't mix freuquentist with Bayesian analyses in the same paper.  However, the Mantel test is a convenient way to evaluate if auto-correlation exists.  An alternative would be to plot a semivariogram of the data, i.e, the semivariance vs the distance. This can be done without computing residuals using the plot function in the `geoR` package.
</div>

The covariance matrix in the process model will use the first distance matrix you created (structured variance) and an identity matrix multiplied by a separate variance component (unstructured variance), $\mathbf{D}$, such that
$$\Sigma_{x,y} = \sigma^2_{unstr}\mathbf{I} + \sigma^2_{str} e^{(\frac{-\mathbf{D}_{x,y}}{\phi})}$$
...
We will provide you will initial values to guide you in setting up the full model. 
...
X = cbind(as.double(X.1[,1]), as.double(X.1[,2]), 
           as.double(X.1[,3]), as.double(X.1[,4]))
...
inits.aspatial <- list(
  list(beta = c(1, 1, 1, 1), tau.p = 0.1, alpha.c = 0.25, tau.c = 81, tau.p.unstr = 1),
  list(beta = c(1.5, 1.5, 1.5, 1.5), tau.p = 1, alpha.c = 0.23, tau.c = 85, tau.p.unstr = 2)
)

inits.spatial <- list(
  list(beta = c(1, 1, 1, 1), tau.p = 0.1, alpha.c = 0.25, tau.c = 81, tau.p.unstr = 1, tau.p.str = 1, phi = 1),
  list(beta = c(1.5, 1.5, 1.5, 1.5), tau.p = 1, alpha.c = 0.23, tau.c = 85, tau.p.unstr = 2, tau.p.str = 2, phi = 2)
) 
```

3. Build and fit the hierarchical model to the experimental data in JAGS. We highly suggest using a small number of iterations as you are getting started with this model (it took 5 minutes to initialize, update, and make coda samples using n.adapt = 250, n.update = 500, n.iter = 1000). Once you get the model running with results that seem reasonable, then you can up your iterations. Some hints:
  + First, fit the model aspatially (no structured variance)
  + You will need to use `dmvnorm()` for the distribution of $\mathbf {z}$. Rather than looping over *i* values of $\mathbf z$, keep the mean and response as a vector. As with `dnorm()`, the variance component needs to be in the form of precision. To do this, take the inverse of your covariance matrix using `inverse()`. 
  + Create an object for the sum of your covariates multiplied by their respective $\beta$ 's. We can call this vector $u$ and can create it using matrix multiplication in JAGS: $u = \mathbf{X}$ %*% $\boldsymbol{\beta}$.
...
As always, include code for posterior predictive checks.
...
cat("
 model{
  ## priors
  ## spatial priors 
  # COMMENT OUT FOR ASPATIAL MODEL
  tau.p.str ~ dgamma(0.001, 0.001)
  sigma2.p.str <- 1 / tau.p.str
  phi ~ dunif(1e-10, 5)
  
  # aspatial priors
  tau.p.unstr ~ dgamma(0.001, 0.001)
  sigma2.p.unstr <- 1/tau.p.unstr
  for(i in 1:p){
     beta[i]~dnorm(0,0.0001)
   }
   
  # informed priors
   tau.c  ~ dgamma(tau_prior[1], tau_prior[2])
   alpha.c ~ dgamma(alpha_prior[1], alpha_prior[2])
  ...
  # COMMENT OUT FOR ASPATIAL MODEL
   ...
   for(i in 1:n){
     for(j in 1:n){
       Cov.mat[i,j]<-sigma2.p.str*exp(-D[i,j]/phi)
     }
   }
   
  # Define identity matrix for sigma.p.unstr
    Id.mat <- Id * sigma2.p.unstr 

  # invert covariance matrix to obtain precision matrix (use in multivariate normal)
   
   Prec.mat <- inverse(Cov.mat + Id.mat)# COMMENT OUT FOR ASPATIAL MODEL
  # Prec.mat <- inverse(Id.mat)
...
#Use fewer iterations when developing model.
n.adapt = 3000
n.update = 15000
n.iter = 30000

# initialize model with 2 chains
jm.hier = jags.model(file="brte_model_jags.R", data=data.spatial, n.adapt = n.adapt, inits=inits.spatial, n.chains=length(inits))
update(jm.hier, n.iter = n.update)
zj.hier = jags.samples(jm.hier, variable.names = c("beta", "alpha.c", "tau.c", "tau.p.str", "phi", "p.sumsq", "tau.p.unstr"), n.iter=n.iter)

# posterior predictive check
mean(zj.hier$p.sumsq)

```
4. Plot the marginal posterior distributions for $\beta_0$, $\beta_\text{diversity}$, $\beta_\text{region}$, $\beta_\text{diversity*region}$, $\sigma^2_{str}$, $\sigma^2_{unstr}$ $\sigma^2_c$, $\alpha^2_c$, and $\phi$. Overlay lines to show 95% credible intervals. Interpret your results: How do source diversity and region influence colonization success of *B. tectorum* founding populations?
```{r, include = key, echo = key, eval = key}
...
# sigma^2_p_str 
plot(density(1/zj.hier$tau.p.str), main = "", xlab=expression(sigma[str]^2), cex.lab =1.3)
abline(v=quantile(1/zj.hier$tau.p.str, c(0.025, 0.975)), col = "red", lty = "dashed")

# sigma^2_p_unstr 
plot(density(1/zj.hier$tau.p.unstr), main = "", xlab=expression(sigma[unstr]^2), cex.lab =1.3)
abline(v=quantile(1/zj.hier$tau.p.unstr, c(0.025, 0.975)), col = "red", lty = "dashed")
...
for (i in 1:length(zj.hier$phi)){
  y[i,] <- covar(phi = zj.hier$phi[i], d = distance, sigma.p.str = 1/zj.hier$tau.p.str[i])
}
...
<div class="hider">
Plots that are less than 10 meters apart covary somewhat. Plots that are within ~4 meters of each other covary considerably.
</div>
