...
zj_a =jags.samples(jm_a,variable.names=c("beta","sigma","epsilon", "mu", "p.sd", "p.mean", "y.new"), n.iter=10000)

Dsel_a = sum((data$y-summary(zj_a$y.new,mean)$stat)^2) + sum((summary(zj_a$y.new,sd)$stat)^2)
Dsel_a
...
# points(data$X[,2],mu[2,],col="red")
# points(data$X[,2],mu[1,],col="blue", lty="dashed")
# points(data$X[,2],mu[3,],col="blue", lty="dashed")
...
zj_a2 =jags.samples(jm_a2,variable.names=c("beta", "epsilon", "sigma", "p.sd", "p.mean", "y.new"), n.iter=n.iter)

Dsel_a2 = sum((data$y-summary(zj_a2$y.new,mean)$stat)^2) + sum((summary(zj_a2$y.new,sd)$stat)^2)
Dsel_a2
...
#### IV. Exercise 2: A spatial model fit to spatially structured data
...
data = list(
  y=as.double(dat$y_structured[idxkeep]),
  y.n = length(as.double(dat$y_structured[idxkeep])),
  X = X[idxkeep,],
  y.p = p,
  y.I = diag(length(idxkeep)),
  D = D.sm,
...
cat("
    ...
    # prec.mat = inverse(sigma^2*y.I)
    # y[] ~ dmnorm(mu[]+ eta ,prec.mat)
    # y.new[] ~ dmnorm(mu[] + eta ,prec.mat)
    # epsilon <- y - mu - eta
    # 
    #Converges much more quickly than adding 0 centered above but does not recover unstructured epsilon Use the block below to estimate parameters. This code coverged with 5000 burn-in and 5000 iterations of three chains.  Use code above to do variogram small number of iterations or run overnight.
    prec.mat = inverse(Cov.mat + sigma^2*y.I)
    y[] ~ dmnorm(mu[],prec.mat)
    y.new[] ~ dmnorm(mu[], prec.mat)
    p.sd <- step(sd(y.new) - sd(y))
    }
...
n.update = 1000
n.iter = 1000
jm_s=jags.model("SpatialJAGS.R", n.adapt = n.adapt, n.chains =  length(inits), inits=inits, data=data)
update(jm_s, n.iter=n.update)
#zc_s = coda.samples(jm_s, variable.names=c("beta","sigma","phi", "mu", "p.sd"), n.iter=n.iter)

zj_s =jags.samples(jm_s,variable.names=c("epsilon","beta","sigma","phi","sigma_sq_struc", "mu", "p.sd", "y.new", "eta"), n.iter=n.iter)

heidel.diag(as.mcmc.list(zj_s$phi))
heidel.diag(as.mcmc.list(zj_s$beta))
heidel.diag(as.mcmc.list(zj_s$sigma))


gelman.diag(as.mcmc.list(zj_s$beta))

plot(as.mcmc.list(zj_s$beta))
Dsel_s = sum((data$y-summary(zj_s$y.new,mean)$stat)^2) + sum((summary(zj_s$y.new,sd)$stat)^2)
Dsel_s
