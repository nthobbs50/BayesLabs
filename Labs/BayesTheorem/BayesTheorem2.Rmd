...
# improved histogram of the data
...
```{r, echo = FALSE}
ui <- fluidPage(
  column(5, sliderInput(inputId = "sampleSize", label = "Choose a sample size", value = 25, min = 1, max = 300),
  sliderInput(inputId = "meanPrior", label = "Choose prior mean", value = 10.2, min = 0, max = 25, step = .1),
  sliderInput(inputId = "sdPrior", label = "Choose prior standard deviation", value = .5, min = .2, max = 10, step = .05)),
column(7, plotOutput(outputId = "plotit"))
)

server <- function(input, output){
  output$plotit <- renderPlot({
    set.seed(3)
    y <- rpois(input$sampleSize, lambda = 6.4)
    mu.prior <- input$meanPrior
    sigma.prior <- input$sdPrior
    step <- .01
    theta <- seq(0, 15, step)
    prior <- function(theta, mu = mu.prior, sigma = sigma.prior) dgamma(theta, mu^2 / sigma^2, mu / sigma^2)
    like <- function(theta, y){
    ...
    par(mfrow=c(1, 1))
    plot(theta, like.scaled, type = "l", col = "red", xlim = c(0,15), xlab = expression(theta), main = "Scaled Overlay", ylab = "Probability density")
