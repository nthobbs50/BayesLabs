...
#### IV. Pooled model
Your first task is to write a simple, "pooled" model where you gloss over differences in sites and fertilizer types and lump everything into a set of x & y pairs.  Study the data statement below and match the variable names in your JAGS code to the left hand side of the = in the data list.  Call the intercept `alpha`, the slope `beta` and use `sigma` to name the standard deviation in the likelihood.
...
Now you will implement the model that allows intercept to vary by group, where each intercept is drawn from a common distribution.Here are the preliminaries to set up the model:
...
Modify your model to include a covariate at the site level, soil carbon content and allow slopes to vary with fertilizer type as developed in the model writing problem #3.

Set up data and initial conditions -- up to you this time.
